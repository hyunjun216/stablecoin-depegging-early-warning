"""
Purpose:
    Compare candidate models after selecting coin-specific imbalance handling.

Input:
    data/ml/best_config.json plus X/Y modeling files for USDC and DAI.

Output:
    data/ml/model_comparison_results.csv, data/ml/model_fold_details.csv, and
    figures/model comparison plots when output paths are configured.

Notes:
    Final presentation model is XGBoost for both USDC and DAI. F2-score and
    Recall are prioritized because missed depegging events are more costly
    than false alarms in this early-warning setting.
"""
...
def load_data(coin):
    X = pd.read_csv(os.path.join(ML_DIR, f"X_{coin.lower()}.csv"))
    Y = pd.read_csv(os.path.join(ML_DIR, f"Y_{coin.lower()}.csv"), header=None).squeeze()
