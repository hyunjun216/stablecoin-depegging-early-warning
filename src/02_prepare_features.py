"""
Purpose:
    Select the final modeling features and create USDC/DAI X and Y matrices.

Input:
    data/processed/df_usdc.csv and data/processed/df_dai.csv.

Output:
    data/ml/X_{coin}.csv, data/ml/Y_{coin}.csv, selected feature lists, and
    feature category metadata.

Notes:
    The final portfolio narrative uses 218 candidate variables and 60 final
    variables. This script applies mechanical filters, outlier clipping,
    collinearity filtering, and tree-based importance selection.
"""
...
def main():
    for coin in ["USDC", "DAI"]:
        ...
        # 저장
        X.to_csv(os.path.join(ML_DIR, f"X_{coin.lower()}.csv"), index=False)
        Y.to_csv(os.path.join(ML_DIR, f"Y_{coin.lower()}.csv"), index=False, header=False)
        pd.DataFrame({"feature": selected}).to_csv(
            os.path.join(ML_DIR, f"selected_features_{coin.lower()}.csv"), index=False
        )
        cat_df.to_csv(
            os.path.join(ML_DIR, f"feature_categories_{coin.lower()}.csv"), index=False
        )

        print(f"\n  저장 완료: X_{coin.lower()}.csv ({X.shape})")
        print(f"  카테고리 분포:")
