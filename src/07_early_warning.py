"""
Purpose:
    Convert model probabilities into a three-stage early warning system.

Input:
    USDC/DAI modeling files and trained or cross-validated XGBoost probability
    outputs generated by the modeling pipeline.

Output:
    data/ml/early_warning_{coin}.csv, threshold summaries, and early-warning
    figures.

Notes:
    Warning levels are Normal, Caution, and Alert. Alert thresholds emphasize
    F2-score and Recall. The final presentation uses USDC WeightOnly+XGBoost
    and DAI ADASYN+XGBoost.
"""
